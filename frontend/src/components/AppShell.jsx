      ...
      <aside className={`fixed inset-y-0 left-0 z-40 hidden border-r border-white/[0.07] bg-slate-900/80 backdrop-blur-xl transition-[width] duration-300 lg:flex lg:flex-col ${collapsed ? 'w-20' : 'w-64'}`}>
        <div className={`flex h-20 items-center border-b border-white/[0.06] ${collapsed ? 'justify-center' : 'px-6'}`}>
          <div className="flex items-center gap-3 overflow-hidden">
            <span className="flex h-10 w-10 flex-none items-center justify-center rounded-xl bg-gradient-to-br from-blue-500 to-violet-600 shadow-lg shadow-blue-950/40"><BrainCircuit className="h-6 w-6" /></span>
        ...
        <div className="relative space-y-1 border-t border-white/[0.06] p-3">
          <AnimatePresence>
            {accountOpen && <motion.div ref={accountMenuRef} initial={{ opacity: 0, y: 8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 8, scale: 0.98 }} className={`absolute bottom-[4.75rem] overflow-hidden rounded-2xl border border-white/10 bg-slate-800 p-2 shadow-2xl shadow-black/40 ${collapsed ? 'left-3 w-56' : 'inset-x-3'}`}>
              <button onClick={() => { setAccountOpen(false); navigate('/settings'); }} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-slate-200 transition hover:bg-white/[0.07]"><Settings className="h-5 w-5 text-slate-400" /><span className="font-medium">Ustawienia</span></button>
              <button disabled className="flex w-full cursor-not-allowed items-center gap-3 rounded-xl px-3 py-3 text-left text-slate-500"><Languages className="h-5 w-5" /><span className="flex-1 font-medium">Język</span><span className="text-xs">Wkrótce</span></button>
              <button disabled className="flex w-full cursor-not-allowed items-center gap-3 rounded-xl px-3 py-3 text-left text-slate-500"><Moon className="h-5 w-5" /><span className="flex-1 font-medium">Motyw</span><span className="text-xs">Wkrótce</span></button>
          ...
          <button ref={accountTriggerRef} onClick={() => setAccountOpen(value => !value)} aria-expanded={accountOpen} aria-haspopup="menu" className={`flex w-full items-center rounded-xl py-2.5 transition hover:bg-white/[0.06] ${collapsed ? 'justify-center px-2' : 'gap-3 px-2.5'}`}>
            <span className="flex h-10 w-10 flex-none items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-violet-600 font-bold shadow-lg shadow-blue-500/20">{userInitial}</span>
            {!collapsed && <><span className="min-w-0 flex-1 text-left"><span className="block text-sm font-semibold text-white">Twoje konto</span><span className="block text-xs text-slate-500">Profil i preferencje</span></span><Settings className="h-5 w-5 text-slate-500" /></>}
          </button>
          <button onClick={() => setCollapsed(value => !value)} className={`group mt-2 flex w-full items-center rounded-xl border border-white/[0.06] py-2 text-slate-400 transition hover:border-white/10 hover:bg-white/[0.04] hover:text-white ${collapsed ? 'justify-center px-2' : 'justify-between pl-3.5 pr-2'}`} aria-label={collapsed ? 'Rozwiń menu' : 'Zwiń menu'}>
            {!collapsed && <span className="text-sm font-medium">Zwiń menu</span>}
        ...
        <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-slate-950/85 px-4 py-3 backdrop-blur-xl sm:px-6 lg:px-8 lg:py-4">
          <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
            <div className="min-w-0">
              <p className="hidden text-xs font-medium uppercase tracking-[0.2em] text-slate-500 sm:block">Centrum wiedzy</p>
              <h1 className="truncate text-2xl font-bold tracking-tight lg:text-3xl">{current.label}</h1>
            </div>
            <div className="flex items-center gap-2 sm:gap-3">
              {hasCreateAction && <button onClick={() => focusCreateForm(activeSection)} className="hidden items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold shadow-lg shadow-blue-950/30 transition hover:bg-blue-500 sm:flex"><Plus className="h-4 w-4" /> Dodaj</button>}
            </div>
          </div>
        </header>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-white/10 bg-slate-900/95 px-4 pb-[max(.65rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl lg:hidden">
        <div className="mx-auto flex max-w-lg items-center justify-around">
          {primaryMobile.map(id => {
            ...
            return <button key={id} onClick={() => go(id)} className={`flex min-w-16 flex-col items-center gap-1 rounded-xl px-3 py-1.5 text-[11px] font-medium transition ${active ? 'text-blue-400' : 'text-slate-500'}`}><Icon className="h-5 w-5" />{item.label}</button>;
          })}
          <button onClick={() => setMoreOpen(true)} className={`flex min-w-16 flex-col items-center gap-1 rounded-xl px-3 py-1.5 text-[11px] font-medium transition ${['tasks', 'links'].includes(activeSection) ? 'text-blue-400' : 'text-slate-500'}`}><Menu className="h-5 w-5" />Więcej</button>
        </div>
      </nav>
      ...
      <AnimatePresence>
        {moreOpen && <>
          ...
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 300 }} className="fixed inset-x-0 bottom-0 z-50 rounded-t-3xl border-t border-white/10 bg-slate-900 p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-2xl lg:hidden">
            <div className="mb-5 flex items-center justify-between"><div><p className="text-lg font-bold">Więcej</p><p className="text-sm text-slate-500">Pozostałe sekcje i konto</p></div><button onClick={() => setMoreOpen(false)} className="rounded-xl bg-white/5 p-2 text-slate-400"><X className="h-5 w-5" /></button></div>
            <div className="grid grid-cols-2 gap-3">
              {[sections[2], sections[4]].map(item => <button key={item.id} onClick={() => go(item.id)} className="flex items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.04] p-4 text-left"><item.icon className="h-5 w-5 text-blue-400" /><span className="font-medium">{item.label}</span></button>)}
              ...
              <button disabled className="flex cursor-not-allowed items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4 text-left text-slate-500"><Languages className="h-5 w-5" /><span className="font-medium">Język · wkrótce</span></button>
              <button disabled className="flex cursor-not-allowed items-center gap-3 rounded-2xl border border-white/[0.07] bg-white/[0.025] p-4 text-left text-slate-500"><Moon className="h-5 w-5" /><span className="font-medium">Motyw · wkrótce</span></button>
            </div>
          </motion.div>
        </>}
      </AnimatePresence>
