...
import { Settings } from 'lucide-react'; // <--- Dodaj 'Settings'
...
const DecksView = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      await api.post('/decks/upload-form', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
      toast.success("Zestaw utworzony! 🚀");
      setDeckName(''); setFiles(null); fetchDecks();
    } catch (err) { toast.error("Błąd wgrywania."); } finally { setIsUploading(false); }
  };
        ...
        {filteredDecks.map(deck => (
          <motion.div key={deck.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-800/50 border border-slate-700 p-6 rounded-2xl hover:border-blue-500/50 transition-all flex flex-col justify-between">
            ...
            <div className="grid grid-cols-2 gap-2 mt-auto">
              ...
              <button
                onClick={() => navigate(`/quiz/${deck.id}`)}   // <--- O TO CHODZIŁO
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
      <div className="lg:col-span-1">
        <div className="bg-slate-800/50 border border-slate-700 p-6 rounded-2xl sticky top-0">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <textarea rows="8" placeholder="Treść notatki..." value={content} onChange={e => setContent(e.target.value)} className="w-full bg-slate-900 border border-slate-700 text-white rounded-xl p-3 focus:border-purple-500 outline-none resize-none"></textarea>
            <div className="flex gap-2">
              <button type="submit" className={`flex-1 py-2 rounded-xl font-medium text-white ${editingId ? 'bg-yellow-600' : 'bg-purple-600'}`}>{editingId ? "Zapisz zmianę" : "Dodaj notatkę"}</button>
      ...
      <div className="lg:col-span-2 flex flex-col gap-6">
        <div className="relative">
          <Search className="absolute left-4 top-3.5 h-5 w-5 text-slate-500" />
          <input type="text" placeholder="Szukaj w notatkach..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-slate-800/80 border border-slate-700 text-white rounded-xl pl-12 pr-4 py-3 focus:border-purple-500 outline-none placeholder-slate-500" />
        </div>
            ...
            return (
              ...
              >
                <div className="flex-1 min-w-0 pr-4">
                  ...
                  {/* POPRAWKA TUTAJ: break-words i break-all */}
                ...
                <div className="flex gap-2">
                  <button
                    onClick={(e) => { e.stopPropagation(); setEditingId(note.id); setTitle(note.title); setContent(note.content); }}
                    className="p-2 bg-slate-900 border border-slate-700 hover:border-yellow-500 text-yellow-500 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                    title="Edytuj"
                  >
                    <Pencil className="h-4 w-4" />
                  </button>
                  <button
                    onClick={async (e) => { e.stopPropagation(); if (window.confirm("Usunąć?")) { await api.delete(`/notes/${note.id}`); fetchNotes(); toast.info("Usunięto"); } }}
                    className="p-2 bg-slate-900 border border-slate-700 hover:border-red-500 text-red-500 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                    title="Usuń"
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {links.map(link => (
          <motion.div key={link.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-slate-800/40 border border-slate-700 p-4 rounded-xl flex items-center justify-between group hover:border-blue-500/50 transition-all hover:bg-slate-800/60">
            ...
            <button onClick={() => deleteLink(link.id)} className="text-slate-600 hover:text-red-400 p-2 opacity-0 group-hover:opacity-100 transition-all hover:bg-white/5 rounded-lg"><Trash2 className="h-5 w-5" /></button>
          </motion.div>
        ))}
      </div>
    </div>
      ...
      <div className="min-h-screen bg-slate-950 flex text-white overflow-hidden font-sans">
        ...
        <main className="flex-1 p-8 overflow-y-auto relative h-screen">
          <header className="mb-8 flex items-center justify-between">
            ...
            {/* --- ZMIANA TUTAJ: Nowy kontener na przycisk i avatar --- */}
            <div className="flex items-center gap-3">
              ...
              {/* Twój stary Avatar */}
              <div className="bg-gradient-to-br from-blue-500 to-purple-600 rounded-full w-10 h-10 flex items-center justify-center text-white font-bold shadow-lg shadow-blue-500/30 border border-white/10">
                {userInitial}
              </div>
          ...
          <AnimatePresence mode="wait">
            <motion.div key={activeTab} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.2 }} className="h-full pb-20">
